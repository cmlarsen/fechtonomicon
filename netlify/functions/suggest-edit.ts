...
const getAllowedOrigin = (origin: string | null | undefined): string | null => {
  ...
  if (origin === 'https://fechtonomicon.netlify.app') {
    return origin;
  }

  // Allow custom domain if configured
  if (origin === process.env.CUSTOM_DOMAIN) {
    return origin;
  }
...
const getCorsHeaders = (origin: string | null | undefined): Record<string, string> => {
...
export const handler: Handler = async (event) => {
  ...
  const { cardId, fieldName, correctedValue, term } = editRequest;
  ...
  try {
    const { prUrl, prNumber } = await createPullRequest(octokit, editRequest, repoOwner, repoName);
